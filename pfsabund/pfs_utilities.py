...
class PFSUtilities():
    ...
    def continuum_normalize(self, pfs=None, wavelength_range=None, sigma_l=0.1,
        ...
        if m < k:
            ...
            initcont = splev(pfs.prop('wvl')[w], tck)
    ...
    def continuum_refinement(self, pfs=None, synth=None, wavelength_range=None,
        npix=100, sigma_l=3., sigma_u=3., maxiter=5, k=3, fit_ranges=[[4100.,9100.]]):
        ...
        if m < k:
            ...
            fluxi = pfs.prop('flux')[w] / pfs.prop('initcont')[w]
            ivari = pfs.prop('ivar')[w] * pfs.prop('initcont')[w]**2
            synthi = synth[w]

            #Mask nonsensical values from the continuum determination
            mask = np.full( len(pfs.prop('wvl')[w]), False, dtype=bool)
            ...
            #Exclude regions outside of bounds of synthesized spectra
            for ranges in fit_ranges:
                mask[np.where((pfs.prop('wvl')[w] < ranges[0]) |\
                              (pfs.prop('wvl')[w] > ranges[1]))] = False

            #Mask the first 4 pixels at the beginning and end of each arm
            mask[0:4] = False; mask[-5:-1] = False
            ...
            refinedcont = pfs.prop('initcont')[w] * noise_spectrum
